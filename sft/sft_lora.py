...
from prompts import (get_newsgroup_data_for_ft, get_boolq_data_for_ft, get_COPA_data_for_ft, get_inference_data_for_ft,
                     get_INF2_data_for_ft,
...
def main(args):
    ...
    if "MNLI" in args.tasks:
        train_datasets.append(get_inference_data_for_ft(
            mode="train", name="MNLI", train_sample_fraction=args.train_sample_fraction,
            max_test_sample=args.max_samples
        )[0])
    ...
    if "QNLI" in args.tasks:
        train_datasets.append(get_INF2_data_for_ft(
            mode="train", name="QNLI", train_sample_fraction=args.train_sample_fraction,
            max_test_sample=args.max_samples
        )[0])
    if "WNLI" in args.tasks:
        train_datasets.append(get_INF2_data_for_ft(
            mode="train", name="WNLI", train_sample_fraction=args.train_sample_fraction,
            max_test_sample=args.max_samples
        )[0])
    ...
    if "newsqa" in args.tasks:
        train_datasets.append(get_QA_data_for_ft(
            mode="train", name="newsqa", train_sample_fraction=args.train_sample_fraction,
            max_test_sample=args.max_samples
        )[0])
    if "hotpot_qa" in args.tasks:
        train_datasets.append(get_QA_data_for_ft(
            mode="train", name="hotpot_qa", train_sample_fraction=args.train_sample_fraction,
            max_test_sample=args.max_samples
        )[0])
    if "squad_v2" in args.tasks:
        train_datasets.append(get_QA_data_for_ft(
            mode="train", name="squad_v2", train_sample_fraction=args.train_sample_fraction,
            max_test_sample=args.max_samples
        )[0])
    if "race_middle" in args.tasks:
        train_datasets.append(
            get_race_data_for_ft(mode="train", level="middle", train_sample_fraction=args.train_sample_fraction,
                                 max_test_sample=args.max_samples)[0])
    if "race_high" in args.tasks:
        train_datasets.append(
            get_race_data_for_ft(mode="train", level="high", train_sample_fraction=args.train_sample_fraction,
                                 max_test_sample=args.max_samples)[0])
    ...
    # ensure shuffle training data
    training_args = TrainingArguments(
        output_dir=results_dir,
        logging_dir=f"{results_dir}/logs",
        num_train_epochs=args.epochs,
        per_device_train_batch_size=16 if args.bnb else 2, #REZA 32 if args.bnb else 4
        gradient_accumulation_steps=4 if args.bnb else 32, #REZA 2 if args.bnb else 16
        gradient_checkpointing=False,
        optim="paged_adamw_32bit" if args.bnb else "adamw_torch",
        logging_steps=100,
        learning_rate=2e-4,
        bf16=True,
        tf32=False, #REZA True
        max_grad_norm=0.3,
        warmup_ratio=0.03,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_ckpt",
                        default="/cpfs01/projects-HDD/cfff-23ba4487e9df_HDD/wangzili/.cache/Llama-2-7b-hf")  # /home/hucx/huchenxu/qzh/model/open-llama/7B "NousResearch/Llama-2-7b-hf"
