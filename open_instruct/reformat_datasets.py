...
def convert_super_ni_data(data_dir, output_dir, zero_shot_examples_per_task=60, few_shot_examples_per_task=20,
                          n_few_shot=2):
    ...
    with open(os.path.join(output_dir, "super_ni_data.jsonl"), "w") as fout:
        for task in train_tasks:
            ...
            if zero_shot_examples_per_task + few_shot_examples_per_task < len(task_data["Instances"]):
                instances = random.sample(task_data["Instances"],
...
def convert_sharegpt_data(data_dir, output_dir, data_file="sharegpt_html_cleaned_and_split_2048.json",
                          num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    with open(os.path.join(data_dir, data_file), "r") as fin:
    ...
    def dfs(reply, messages, valid_sequences):
        ...
        if reply["role"] == "assistant":
            ...
            if not reply["replies"]:  # leaf node
                ...
                child_replies = child_replies if top_k_reply is None else sorted(child_replies,
                                                                                 key=lambda x: x["labels"]["quality"][
                                                                                     "value"], reverse=True)[
                                                                          :top_k_reply]
                for child in child_replies:
            ...
            child_replies = child_replies if top_k_reply is None else sorted(child_replies,
                                                                             key=lambda x: x["labels"]["quality"][
                                                                                 "value"], reverse=True)[:top_k_reply]
            for child in child_replies:
                dfs(child, messages, valid_sequences)
            messages.pop()
        else:
        ...
        if dataset == "tulu_v1":
            ...
            with open(os.path.join(args.output_dir, "tulu_v1", "tulu_v1_data.jsonl"), "w") as fout:
                for subset in all_subsets:
                    ...
                    with open(os.path.join(args.output_dir, "tulu_v1", subset, f"{dataset_name}_data.jsonl"),
                              "r") as fin:
                        for line in fin:
                            fout.write(line)
            ...
            with open(os.path.join(args.output_dir, "tulu_v2", "tulu_v2_data.jsonl"), "w") as fout, \
                ...
                for subset in all_subsets:
                    dataset_name = subset[:-len("_subset")]
                    with open(os.path.join(args.output_dir, "tulu_v2", subset, f"{dataset_name}_data.jsonl"),
                              "r") as fin:
                        for line in fin:
            ...
            globals()[f"convert_{dataset}_data"](os.path.join(args.raw_data_dir, dataset),
